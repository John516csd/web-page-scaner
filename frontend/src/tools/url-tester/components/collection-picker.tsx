...
import { Plus, MoreHorizontal, Pencil, Trash2, FolderOpen, Clock } from "lucide-react";
import type { TestCollection } from "../types";
...
interface CollectionPickerProps {
  ...
  onSelect: (collection: TestCollection) => void;
...
export function CollectionPicker({
  ...
  const [renameTarget, setRenameTarget] = useState<TestCollection | null>(null);
  ...
  const openRename = (col: TestCollection) => {
    setRenameTarget(col);
    setNewName(col.name);
    setRenameOpen(true);
  };
          ...
          {collections.map((col) => {
            ...
            return (
              <SelectItem key={col.id} value={col.id}>
                {hasSchedule && "⏱ "}{col.name} ({col.testCases.length})
              </SelectItem>
            );
          })}
        </SelectContent>
      ...
      {activeId && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" disabled={disabled}>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button size="sm" onClick={handleRename} disabled={!newName.trim() || saving}>
                {saving ? "保存中..." : "保存"}
              </Button>
            </div>
          </div>
