...
export async function runAcceptanceWorkbench(
  ...
  for (const suite of suites) {
    ...
    const section = await runSuite(suite, stageForSuite(suite, stages), {
      ...context,
      progress: createSuiteItemProgress(suite, emit),
    });
...
interface SuiteContext {
  ...
  e2eCollections?: E2ETestCollection[];
  e2eExecutor: (testCase: E2ETestCase) => Promise<E2ETestResult>;
  progress?: SuiteItemProgress;
}

interface SuiteItemProgress {
  setTotal: (total: number) => void;
  run: (
    descriptor: { id?: number | string; name: string },
    callback: () => Promise<AcceptanceItem>
  ) => Promise<AcceptanceItem>;
}

function createSuiteItemProgress(
  suite: AcceptanceSuite,
  emit?: ProgressEmitter
): SuiteItemProgress | undefined {
  ...
  return {
    ...
    async run(descriptor, callback) {
      const index = nextIndex++;
      const itemId = descriptor.id || descriptor.name;
      const data = {
        kind: 'suite-item',
        suite,
        itemId,
      ...
      try {
        ...
        emit({
          type: 'progress',
          step: suite,
          status: result.status === 'fail' || result.status === 'blocked' ? 'error' : 'done',
          message: result.name,
          data: {
            ...data,
            itemId: result.id,
...
function setSuiteProgressTotal(context: SuiteContext, total: number) {
  context.progress?.setTotal(total);
}

async function trackSuiteItem(
  context: SuiteContext,
  descriptor: { id?: number | string; name: string },
  callback: () => Promise<AcceptanceItem>
): Promise<AcceptanceItem> {
  if (!context.progress) return callback();
  return context.progress.run(descriptor, callback);
}

async function checkBuildReadiness(context: SuiteContext): Promise<AcceptanceItem[]> {
  if (!context.sourceRepoPath) {
    setSuiteProgressTotal(context, 1);
    return [
      await trackSuiteItem(context, { id: 'source-repo-path', name: 'source repo path' }, async () =>
        manualItem('source repo path', 'sourceRepoPath not provided; local build readiness requires evidence', 'P1')
      ),
    ];
  }

  setSuiteProgressTotal(context, 5);
  ...
  try {
    ...
    for (const script of ['build', 'sst:deploy:test', 'sst:deploy']) {
      items.push(await trackSuiteItem(context, { id: `package-script-${script}`, name: `package script ${script}` }, async () =>
        item({
    ...
    items.push(await trackSuiteItem(context, { id: 'package-json-readable', name: 'package.json readable' }, async () =>
      failItem('package.json readable', packageJsonPath, 'package.json can be parsed', error, 'P0')
    ));
  }

  items.push(await trackSuiteItem(context, { id: 'sst-config', name: 'sst.config.ts exists' }, () =>
    fileExistsItem('sst.config.ts exists', sstPath, 'P1')
  ));
  items.push(await trackSuiteItem(context, { id: 'github-workflows', name: 'GitHub workflow directory exists' }, () =>
    fileExistsItem('GitHub workflow directory exists', workflowsPath, 'P1')
  ));
  return items;
}
...
async function checkSmoke(context: SuiteContext): Promise<AcceptanceItem[]> {
  ...
  setSuiteProgressTotal(context, cases.length);
  return await runHttpItems(cases, 'P0', context);
}
...
async function checkRouting(context: SuiteContext): Promise<AcceptanceItem[]> {
  ...
  const cases: Array<{ testCase: Parameters<typeof runAcceptanceHttpCase>[0]; severity: AcceptanceItem['severity'] }> = [];
  ...
  setSuiteProgressTotal(context, cases.length);
  return await runHttpItems(cases.map((entry) => entry.testCase), 'P0', context, (testCase) =>
    cases.find((entry) => entry.testCase === testCase)?.severity || 'P2'
  );
}

async function checkSeoGeo(context: SuiteContext): Promise<AcceptanceItem[]> {
  setSuiteProgressTotal(context, 1 + (context.env === 'test' ? 2 : 1) + 1 + 1 + 2 + (context.env === 'production' ? 1 : 0) + 6);
  const items: AcceptanceItem[] = [];
  const robotsUrl = buildUrl(context.baseUrl, '/robots.txt');
  const robots = await trackSuiteItem(context, { id: 'robots', name: 'robots.txt' }, () =>
    httpItem({ id: 'robots', name: 'robots.txt', url: robotsUrl, expectedStatus: 200 }, 'P0')
  );
  items.push(robots);
  ...
  try {
    ...
    if (context.env === 'test') {
      items.push(await trackSuiteItem(context, { id: 'test-robots-blocks-indexing', name: 'test robots blocks indexing' }, async () =>
        item({
      ...
      items.push(await trackSuiteItem(context, { id: 'test-robots-production-sitemap', name: 'test robots does not expose production sitemap' }, async () =>
        item({
      ...
      items.push(await trackSuiteItem(context, { id: 'production-robots-allows-indexing', name: 'production robots allows indexing' }, async () =>
        item({
    ...
    items.push(await trackSuiteItem(context, { id: 'robots-policy-parse', name: 'robots policy parse' }, async () =>
      failItem('robots policy parse', robotsUrl, 'robots policy readable', error, 'P0')
    ));
  }

  const sitemapUrl = buildUrl(context.baseUrl, '/sitemap-0.xml');
  items.push(await trackSuiteItem(context, { id: 'sitemap-0-valid', name: 'sitemap-0.xml valid' }, async () => {
    try {
      ...
      return failItem('sitemap-0.xml valid', sitemapUrl, '200 and valid XML', error, 'P0');
    }
  }));

  items.push(await trackSuiteItem(context, { id: '/sitemap.xml', name: '/sitemap.xml compatibility redirect' }, () => httpItem({
    id: '/sitemap.xml',
  ...
  for (const path of ['/llms.txt', '/llms-full.txt']) {
    items.push(await trackSuiteItem(context, { id: path, name: path }, () => httpItem({
      id: path,
      name: path,
      url: buildUrl(context.baseUrl, path),
      expectedStatus: 200,
  ...
  if (context.env === 'production') {
    items.push(await trackSuiteItem(context, { id: 'indexnow', name: 'IndexNow key' }, () => httpItem({
      id: 'indexnow',
      name: 'IndexNow key',
  ...
  for (const path of SEO_PARITY_PATHS.slice(0, 6)) {
    items.push(await trackSuiteItem(context, { id: `head-parity-${path}`, name: `head parity ${path}` }, () =>
      headParityItem(context, path)
    ));
  }
...
async function checkPageParity(context: SuiteContext): Promise<AcceptanceItem[]> {
  ...
  setSuiteProgressTotal(context, paths.length);
  const items: AcceptanceItem[] = [];
  for (const path of paths) {
    items.push(await trackSuiteItem(context, { id: `page-parity-${path}`, name: `page parity ${path}` }, async () => {
      ...
      try {
        ...
        return failItem(`page parity ${path}`, urlB, 'Gatsby/Next diff succeeds', error, 'P1', Date.now() - start);
      }
    }));
  }
  return items;
...
async function checkCmsStoryblok(context: SuiteContext): Promise<AcceptanceItem[]> {
  const cases = CMS_SAMPLE_PATHS.map((path) => ({
    id: `cms-${path}`,
    name: `CMS page ${path}`,
    url: buildUrl(context.baseUrl, path),
    expectedStatus: 200,
  ...
  setSuiteProgressTotal(context, cases.length + 1);
  const items = await runHttpItems(cases, 'P1', context);
  items.push(await trackSuiteItem(context, { id: 'cms-404', name: 'missing CMS slug returns 404' }, () => httpItem({
    id: 'cms-404',
...
async function checkI18n(context: SuiteContext): Promise<AcceptanceItem[]> {
  ...
  const items: AcceptanceItem[] = [];
  for (const path of paths) {
    items.push(await trackSuiteItem(context, { id: `i18n-${path}`, name: `i18n ${path}` }, async () => {
...
async function checkFunctionalE2E(context: SuiteContext): Promise<AcceptanceItem[]> {
  const items: AcceptanceItem[] = [];
  const testCases = buildMigrationFunctionalE2ECases(context);
  setSuiteProgressTotal(context, testCases.length);
  for (const testCase of testCases) {
    items.push(await trackSuiteItem(context, { id: `functional-e2e-${testCase.id}`, name: testCase.name }, async () => {
      const result = await context.e2eExecutor(testCase);
      const classified = classifyFunctionalE2EResult(result);
      return item({
        id: `functional-e2e-${result.testCase.id}`,
...
function buildMigrationFunctionalE2ECases(context: Pick<SuiteContext, 'baseUrl'>): E2ETestCase[] {
  return [
    {
      id: 'audio-to-text-shell',
      name: 'Audio to Text tool shell readiness',
      url: buildUrl(context.baseUrl, '/en/tools/audio-to-text-converter'),
      timeout: 45000,
      script: audioToTextShellScript(),
      tags: ['migration-acceptance', 'tool-shell'],
    },
    {
      id: 'audio-to-text-paste-link',
      name: 'Audio to Text paste link readiness',
      url: buildUrl(context.baseUrl, '/en/tools/audio-to-text-converter'),
      timeout: 60000,
      script: audioToTextPasteLinkScript(),
      tags: ['migration-acceptance', 'tool-submit'],
    },
    {
      id: 'youtube-summarizer-submit',
      name: 'YouTube Summarizer submit readiness',
      url: buildUrl(context.baseUrl, '/en/tools/youtube-video-summarizer'),
      timeout: 75000,
      script: youtubeSummarizerScript(),
      tags: ['migration-acceptance', 'tool-submit'],
    },
  ];
}

function audioToTextShellScript(): string {
  return `
    await page.locator('body').waitFor({ state: 'visible', timeout: 10000 });
    const uploadArea = page.locator('.ant-upload-drag, input[type="file"]').first();
    await uploadArea.waitFor({ state: 'visible', timeout: 15000 });
    const pasteLinkTab = page.getByText(/paste a link/i).first();
    await pasteLinkTab.waitFor({ state: 'visible', timeout: 10000 });
    const title = await page.title();
    assert(title.length > 0, 'page title should not be empty');
    await capture('audio shell ready', {
      selector: '.ant-upload-drag, input[type="file"]',
      status: 'success',
      metadata: {
        consoleMessage: 'acceptance-status=pass; visibleState=form-ready; apiStatus=none'
      }
    });
  `;
}

function audioToTextPasteLinkScript(): string {
  return `
    const apiResponses = [];
    page.on('response', (response) => {
      const url = response.url();
      if (
        url.includes('/file-transcribe') ||
    ...
    await page.locator('body').waitFor({ state: 'visible', timeout: 10000 });
    const pasteLinkTab = page.getByText(/paste a link/i).first();
    await pasteLinkTab.waitFor({ state: 'visible', timeout: 15000 });
    await pasteLinkTab.click();

    const urlInput = page.locator('input[type="text"], input:not([type]), textarea').first();
    await urlInput.waitFor({ state: 'visible', timeout: 10000 });
    await urlInput.fill('https://www.youtube.com/watch?v=dQw4w9WgXcQ');
    await page.waitForTimeout(500);

    const submitButton = page.getByRole('button', { name: /transcribe|confirm/i }).last();
    await submitButton.waitFor({ state: 'visible', timeout: 10000 });
    const isDisabled = await submitButton.evaluate((element) => {
    ...
    if (isDisabled) {
      await capture('audio paste link submit disabled', {
        selector: 'button',
        status: 'warning',
        metadata: {
          consoleMessage: 'acceptance-status=warn; visibleState=submit-disabled; apiStatus=none'
        }
      });
    } else {
      await submitButton.click();
      let visibleState = await Promise.race([
        page.waitForURL(
          (url) => url.href.includes('from=official') || url.hostname.includes('app.notta.ai'),
          { timeout: 14000 }
        ).then(() => 'redirected'),
        page.locator('[role="dialog"], [class*="dialog"], .ant-message-notice').first()
          .waitFor({ state: 'visible', timeout: 14000 })
          .then(() => 'message-or-dialog'),
      ]).catch(() => 'none');

      if (visibleState === 'none') {
        const stillLoading = await page.locator('[class*="loading"], [class*="skeleton"], [class*="progress"]').first()
          .isVisible()
          .catch(() => false);
        visibleState = stillLoading ? 'loading' : 'none';
      }

      await page.waitForTimeout(2500);
      const badApi = apiResponses.find((entry) => /^[45]\\d\\d /.test(entry));
      const status = badApi || visibleState === 'message-or-dialog' || visibleState === 'loading' || visibleState === 'none'
        ? 'warning'
        : 'success';
      await capture('audio paste link terminal state', {
        selector: '[role="dialog"], [class*="dialog"], .ant-message-notice, [class*="loading"], [class*="skeleton"], [class*="progress"]',
        status,
        metadata: {
          networkRequest: badApi || apiResponses[apiResponses.length - 1] || 'none',
          consoleMessage: 'acceptance-status=' + (status === 'warning' ? 'warn' : 'pass') +
            '; visibleState=' + visibleState +
            '; apiStatus=' + (badApi || 'none')
        }
      });
    }
  `;
...
function youtubeSummarizerScript(): string {
  return `
    const apiResponses = [];
    page.on('response', (response) => {
      const url = response.url();
      if (
        url.includes('/subtitle-summarize') ||
        url.includes('/validate_youtube_link') ||
        url.includes('execute-api') ||
        url.includes('/v1/')
      ) {
        apiResponses.push(response.status() + ' ' + url);
    ...
    await page.locator('body').waitFor({ state: 'visible', timeout: 10000 });
    const input = page.getByPlaceholder(/paste.*link.*here/i).first();
    await input.waitFor({ state: 'visible', timeout: 15000 });
    await input.fill('https://www.youtube.com/watch?v=dQw4w9WgXcQ');
    await page.waitForTimeout(500);

    const submitButton = page.getByRole('button', { name: /generate.*transcript|summarize|generate/i }).first();
    await submitButton.waitFor({ state: 'visible', timeout: 10000 });
    const isDisabled = await submitButton.evaluate((element) => {
      const button = element;
      return Boolean(button.disabled) ||
    ...
    if (isDisabled) {
      await capture('youtube submit disabled', {
        selector: 'button',
        status: 'warning',
        metadata: {
          consoleMessage: 'acceptance-status=warn; visibleState=submit-disabled; apiStatus=none'
      ...
      await submitButton.click();
      let visibleState = await Promise.race([
        page.locator('iframe[src*="youtube-video-summarizer"], [class*="result_page"]')
          .waitFor({ state: 'visible', timeout: 16000 })
          .then(() => 'result'),
        page.getByText(/summary will be ready soon|create.*account|upgrade.*pro|invalid.*youtube.*link|error|failed/i)
          .waitFor({ state: 'visible', timeout: 16000 })
          .then(() => 'message'),
      ...
      if (visibleState === 'none') {
        const stillLoading = await page.locator('[class*="loading"], [class*="loading_icon"], [class*="skeleton"]')
          .first()
          .isVisible()
          .catch(() => false);
        visibleState = stillLoading ? 'loading' : 'none';
      ...
      await page.waitForTimeout(2500);
      const badApi = apiResponses.find((entry) => /^[45]\\d\\d /.test(entry));
      const status = badApi || visibleState === 'message' || visibleState === 'dialog' || visibleState === 'loading' || visibleState === 'none'
        ? 'warning'
        : 'success';
      await capture('youtube terminal state', {
        selector: 'iframe[src*="youtube-video-summarizer"], [class*="result_page"], [role="dialog"], [class*="dialog"], [class*="loading"]',
        status,
        metadata: {
          networkRequest: badApi || apiResponses[apiResponses.length - 1] || 'none',
          consoleMessage: 'acceptance-status=' + (status === 'warning' ? 'warn' : 'pass') +
...
function classifyFunctionalE2EResult(result: E2ETestResult): {
  ...
  const signal = extractFunctionalE2ESignal(result);
  const fatalConsoleError = findFatalConsoleError(result.consoleLogs || []);
  const visibleState = signal.visibleState || (result.passed ? 'form-ready' : 'unknown');
  const apiStatus = signal.apiStatus || 'none';
  const actual = [
    result.passed ? 'page loaded' : 'script failed',
  ...
  if (!result.passed) {
    return {
      status: 'fail',
      severity: 'P1',
      expected: 'tool page loads and core migration flow is inspectable',
      actual,
      failureReason: summarizeFunctionalE2EError(result.error),
    };
  }
  ...
  if (fatalConsoleError) {
    return {
      status: 'fail',
      severity: 'P1',
      expected: 'tool page loads without fatal console errors',
      actual,
      failureReason: fatalConsoleError,
    };
  }
  ...
  if (signal.status === 'warn') {
    return {
      status: 'warn',
      severity: 'P2',
      expected: 'tool page loads, core controls are usable, and API outcome is explicit',
      actual,
      failureReason: summarizeFunctionalE2EWarning(signal),
    };
  }
  ...
  return {
    status: 'pass',
    severity: 'P2',
    expected: 'tool page loads and core controls are ready',
    actual,
  };
}

function extractFunctionalE2ESignal(result: E2ETestResult): {
  status?: 'pass' | 'warn';
  visibleState?: string;
  apiStatus?: string;
} {
  const messages = [
    ...(result.consoleLogs || []),
    ...(result.screenshots || []).map((screenshot) => screenshot.metadata?.consoleMessage || ''),
  ].filter(Boolean);

  for (const message of messages.reverse()) {
    const markerIndex = message.indexOf('acceptance-status=');
    if (markerIndex < 0) continue;
    const signalText = message.slice(markerIndex);
    const signal: { status?: 'pass' | 'warn'; visibleState?: string; apiStatus?: string } = {};
    for (const part of signalText.split(';')) {
      const [rawKey, ...rawValue] = part.trim().split('=');
      const value = rawValue.join('=').trim();
      if (rawKey === 'acceptance-status' && (value === 'pass' || value === 'warn')) signal.status = value;
      if (rawKey === 'visibleState') signal.visibleState = value;
      if (rawKey === 'apiStatus') signal.apiStatus = value;
    }
    return signal;
  }

  const warningCapture = (result.screenshots || []).find((screenshot) => screenshot.status === 'warning');
  return warningCapture ? { status: 'warn', visibleState: warningCapture.step, apiStatus: 'unknown' } : {};
}

function findFatalConsoleError(consoleLogs: string[]): string | undefined {
  return consoleLogs.find((log) => {
    if (!log.startsWith('[error]')) return false;
    if (/status code 405|server responded with a status of 405|AxiosError/i.test(log)) return false;
    if (/Failed to load resource/i.test(log)) return false;
    return /TypeError|ReferenceError|SyntaxError|Cannot read|Unhandled|Minified React error/i.test(log);
  });
}

function summarizeFunctionalE2EError(error?: string): string {
  if (!error) return 'tool flow failed without a detailed error';
  if (/Timeout/i.test(error)) {
    return `timed out waiting for required migration UI state: ${error}`;
  }
  return error;
}

function summarizeFunctionalE2EWarning(signal: { visibleState?: string; apiStatus?: string }): string {
  const parts = [];
  if (signal.visibleState) parts.push(`visible state: ${signal.visibleState}`);
  if (signal.apiStatus && signal.apiStatus !== 'none') parts.push(`api status: ${signal.apiStatus}`);
  return parts.length ? parts.join('; ') : 'tool flow reached a warning state';
}
...
async function checkVisualResponsive(context: SuiteContext): Promise<AcceptanceItem[]> {
  ...
  const items: AcceptanceItem[] = [];
  const checks = ['/', '/pricing', '/en/pricing', '/tools/ai-summary'].flatMap((path) =>
    [
      { name: 'desktop', width: 1440, height: 900 },
      { name: 'mobile', width: 375, height: 812 },
    ].map((viewport) => ({ path, viewport }))
  );
  setSuiteProgressTotal(context, checks.length);
  for (const check of checks) {
    items.push(await trackSuiteItem(
      context,
      { id: `${check.viewport.name}-${check.path}`, name: `${check.viewport.name} no horizontal scroll ${check.path}` },
      async () => {
        const contextBrowser = await browser.newContext({ viewport: { width: check.viewport.width, height: check.viewport.height } });
        const page = await contextBrowser.newPage();
        const url = buildUrl(context.baseUrl, check.path);
        const start = Date.now();
        try {
          ...
          return item({
            name: `${check.viewport.name} no horizontal scroll ${check.path}`,
            url,
            status: ok ? 'pass' : 'fail',
            severity: 'P1',
          ...
          return failItem(`${check.viewport.name} responsive ${check.path}`, url, 'page renders without overflow', error, 'P1', Date.now() - start);
        } finally {
...
async function checkPerformance(context: SuiteContext): Promise<AcceptanceItem[]> {
  const paths = PERFORMANCE_PATHS.slice(0, 4);
  setSuiteProgressTotal(context, paths.length);
  const items: AcceptanceItem[] = [];
  for (const path of paths) {
    items.push(await trackSuiteItem(context, { id: `performance-${path}`, name: `performance timing ${path}` }, async () => {
      const url = buildUrl(context.baseUrl, path);
      const start = Date.now();
      try {
        const response = await fetch(url);
...
async function checkAnalytics(context: SuiteContext): Promise<AcceptanceItem[]> {
  ...
  const start = Date.now();
  return [await trackSuiteItem(context, { id: 'analytics-bootstrap', name: 'analytics bootstrap' }, async () => {
    try {
      await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
...
async function checkAssets(context: SuiteContext): Promise<AcceptanceItem[]> {
  ...
  const url = buildUrl(context.baseUrl, '/');
  const start = Date.now();
  return [await trackSuiteItem(context, { id: 'homepage-static-assets', name: 'homepage static assets' }, async () => {
    try {
...
async function runHttpItems(
  cases: Parameters<typeof runAcceptanceHttpCase>[0][],
  severity: AcceptanceItem['severity'],
  context?: SuiteContext,
  severityFor?: (testCase: Parameters<typeof runAcceptanceHttpCase>[0]) => AcceptanceItem['severity']
): Promise<AcceptanceItem[]> {
  const items: AcceptanceItem[] = [];
  for (const testCase of cases) {
    items.push(await trackSuiteItem(context || ({} as SuiteContext), {
      id: testCase.id,
      name: testCase.name,
    }, () => httpItem(testCase, severityFor?.(testCase) || severity || 'P2')));
  }
  return items;
}
